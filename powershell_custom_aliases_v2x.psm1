## powershell_custom_aliases_v2x.psm1 ##
...
Function _addAlias
{
  param(
    ...
    [string]$drive_letter = $(Read-Host "Enter a drive letter to search, or press [Enter] to search all available drives")
  )

  # getting the drives to search on
  $drives = Get-PSDrive
  Write-Host "Here is the drive_letter var: $($drive_letter)"
  if ($drive_letter -ne "") {
    $drives = @("$drive_letter")
  }
